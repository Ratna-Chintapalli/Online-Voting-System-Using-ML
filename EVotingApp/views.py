...
def train_knn_classifier(train_dir, n_neighbors=3):
    X = []
    y = []
...
def getOutput(status):
    ...
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'evoting_new',charset='utf8')
    with con:
        cur = con.cursor()
...
def predict_from_image(knn_clf, image_path, distance_threshold=0.5):
    image = face_recognition.load_image_file(image_path)
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'evoting_new',charset='utf8')
